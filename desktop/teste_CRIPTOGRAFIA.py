...
def inversa_matriz_2x2(matriz):
    ...
    inversa = [
        [matriz[1][1] * inv_det % 26, -matriz[0][1] * inv_det % 26],
        [-matriz[1][0] * inv_det % 26, matriz[0][0] * inv_det % 26]
    ]
    return inversa

def multiplicar_vetor_matriz(vetor, matriz):
    return [
        (vetor[0] * matriz[0][0] + vetor[1] * matriz[0][1]) % 26,
        (vetor[0] * matriz[1][0] + vetor[1] * matriz[1][1]) % 26
    ]

# Converter cada caractere em um número usando a posição no alfabeto. Ex.: A --> 0; B --> 1
def char_para_num(c):
    return ord(c) - ord('A')    
...
def string_para_vetores(s):
    s = s.upper().replace(" ", "") # Converte para maiúsculas e remove espaços
    if len(s) % 2 != 0:
        s += 'X'  # Adiciona 'X' se a string tiver um número ímpar de caracteres
    vetores = []
    for i in range(0, len(s), 2):
        vetores.append([char_para_num(c) for c in s[i:i+2]])
    return vetores

def vetores_para_string(vetores):
    s = ""
    for vetor in vetores:
        s += "".join(num_para_char(num) for num in vetor)
    return s

def criptografar_mensagem(mensagem, matriz_chave):
    vetores_mensagem = string_para_vetores(mensagem)
    vetores_cifrados = []
    for vetor in vetores_mensagem:
        vetor_cifrado = multiplicar_vetor_matriz(vetor, matriz_chave)
        vetores_cifrados.append(vetor_cifrado)
    return vetores_para_string(vetores_cifrados)
...
def cadastrar_produto():
    ...
    # Criptografar a descrição do produto
    matriz_chave = [[1, 2], [3, 4]]  # Exemplo de matriz chave
    desc_prod_criptografada = criptografar_mensagem(desc_prod, matriz_chave)
    ...
    #Inserir no banco: 
    cursor.execute("INSERT INTO Estoque  VALUES (:id_prod, :nome_prod, :desc_prod_criptografada, :custo_prod, :custo_fixo, :comissao, :imposto, :rentabilidade)", {'id_prod' : id_prod, 'nome_prod': nome_prod , 'desc_prod_criptografada' : desc_prod_criptografada , 'custo_prod': custo_prod , 'custo_fixo': custo_fixo, 'comissao' : comissao, 'imposto' : imposto, 'rentabilidade' : rentabilidade} )
    connection.commit()
    connection.close()
